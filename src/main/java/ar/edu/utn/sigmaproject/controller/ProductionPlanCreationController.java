...
public class ProductionPlanCreationController extends SelectorComposer<Component> {
	...
	private void refreshOrder() {
		if (currentOrder != null) {
			...
			orderBandbox.setValue("");// borramos el text del producto
										// seleccionado
			orderPopupListbox.clearSelection();// deseleccionamos la tabla
		}
	}

	private void refreshOrderPopupList() {// el popup se actualiza en base a los
											// detalles
		orderPopupList = orderRepository.findByCurrentStateType(orderStateTypeRepository.findByName("iniciado"));// se
																													// buscan
																													// los
																													// pedidos
																													// que
																													...
																													// estado
																													// iniciado)
		for (ProductionPlanDetail productionPlanDetail : currentProductionPlanDetailList) {// no
																							// debe
																							// contener
																							// los
																							// pedidos
			...
			orderPopupList.remove(aux);// sacamos los pedidos que estan en la
										// currentProductionPlanDetailList del
										// popup
		}
		// agregamos el pedido que se elimin� y al ser de un plan que se esta
		// editando, no aparece en la lista
		// [aca deberia agregarse a la lista los pedidos que esten en el
		// ProductionPlanDetailList serializado pero no esten en el
		// currentProductionPlanDetailList (solo si se esta editando un plan)]
	...
	public void saveProductionPlan() {
		...
		if (currentProductionPlan == null) { // es un plan nuevo
			...
			currentProductionPlan.setDetails(currentProductionPlanDetailList);
	...
	public void addOrder() {
		// el detalle debe tener id de plan nulo ya que se lo agrega al guardar
		// o actualizar el plan
	...
	private void refreshViewProductionPlan() {
		...
		if (currentProductionPlan == null) {// nuevo plan de produccion
			...
			productionPlanStateTypeListModel
					.addToSelection(productionPlanStateTypeRepository.findByName("iniciado"));
			productionPlanStateTypeSelectBox.setModel(productionPlanStateTypeListModel);
			...
			currentProductionPlanDetailList = currentProductionPlan.getDetails();
	...
	private void refreshProductTotalList() {
		productTotalList = new ArrayList<ProductTotal>();// se empieza con una
															// lista vacia
		for (ProductionPlanDetail auxProductionPlanDetail : currentProductionPlanDetailList) {
			Order auxOrder = auxProductionPlanDetail.getOrder();
			for (OrderDetail auxOrderDetail : auxOrder.getDetails()) {
				// por cada detalle del pedido,
				// observamos si el producto ya esta en la lista, si lo esta sumamos
				// su cantidad y, si no esta lo agregamos
				Boolean is_in_list = false;
				Integer order_detail_units = auxOrderDetail.getUnits();
				for (ProductTotal productTotal : productTotalList) {
					if (productTotal.equals(auxOrderDetail.getProduct())) {// si esta
						is_in_list = true;
						productTotal.setTotalUnits(productTotal.getTotalUnits() + order_detail_units);// sumamos
																										// su
																										// cantidad
																										// con
																										// la
				...
				if (is_in_list == false) {// no esta, por lo tanto agregamos el
											// producto a la lista total
					ProductTotal productTotal = new ProductTotal(auxOrderDetail.getProduct());
					productTotal.setTotalUnits(order_detail_units);// el primer
																	// valor son
																	// el total
																	// de
																	// unidades
			...
			// si es el primer loop del productionPlanDetailList entonces la
			// lista productTotalList deberia estar llena solo con los productos
			// del primer pedido, en el siguiente loop se sumaran los que ya
			// estan y agregaran los nuevos
		}
		// aqui deberia llegar con el productTotalList lleno con todos los
		// productos sin repetir y con el total, que conforman el plan de
		// produccion
	}

	public String getClientName(Client aux) {
		return aux.getName();
	}

	public String getClientNameByOrderId(Order auxOrder) {
		return auxOrder.getClient().getName();
	}

	public int getProductTotalUnits(Product product) {
		int product_total_units = 0;
		for (ProductTotal productTotal : productTotalList) {// buscamos el total
															// de unidades
	...
	public BigDecimal getProductTotalPrice(Product product) {
		...
		// TODO: esta funcion es incorrecta pq agarra el valor actual del producto cuando deberia 
		// ser el valor en el pedido
		return getTotalPrice(product_total_units, product.getPrice());
	}
